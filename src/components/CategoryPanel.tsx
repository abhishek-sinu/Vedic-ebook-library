...
const CategoryPanel: React.FC<CategoryPanelProps & { bookChapters?: { text: string; wordIndex: number }[] }> = ({
  ...
  // Collect all category/subcategory node IDs recursively, deduplicating with a Set
  function getAllCategoryIds(nodes: any[], seen = new Set<string>()): string[] {
    const ids: string[] = [];
    for (const node of nodes) {
      ...
      if (node.children && node.children.length > 0) {
        ids.push(...getAllCategoryIds(node.children, seen));
      }
    }
    return ids;
  }
